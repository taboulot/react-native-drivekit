...
{
    [self isTripRunning:resolve reject:reject]; 
}

- (void)isTripRunning:(RCTPromiseResolveBlock)resolve reject:(RCTPromiseRejectBlock)reject {
    NSNumber* tripRunning = [RNDriveKitTripAnalysisWrapper.shared isTripRunning];
    resolve(tripRunning);
}
